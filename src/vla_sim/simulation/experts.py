"""Privileged experts for collecting three-task simulation demonstrations."""
...
from vla_sim.simulation.scenes import COLOR_PICK_COLORS
...
class HeuristicExpertConfig:
    """Tunable values shared by the three OSC-position heuristics."""
...
class HeuristicPickExpert:
    ...
    @property
    def done(self) -> bool:
        return self.phase is PickPhase.DONE
    ...
    def act(self, raw_observation: Mapping[str, Any]) -> NDArray[np.float32]:
        ...
        if self.phase is PickPhase.APPROACH:
            target = cube.copy()
            ...
            if np.linalg.norm(target - eef) <= self.config.position_tolerance_m:
                self.phase = PickPhase.DESCEND

        elif self.phase is PickPhase.DESCEND:
            target = cube.copy()
        ...
        elif self.phase is PickPhase.LIFT:
            action[-1] = self.config.close_command
        ...
        elif self.phase is PickPhase.DONE:
...
class HeuristicColorPickExpert(HeuristicPickExpert):
...
class HeuristicPickPlaceExpert:
    ...
    def act(self, observation: Mapping[str, Any]) -> NDArray[np.float32]:
        cube = HeuristicPickExpert._vector(observation, "cube_pos", 3)
        target = HeuristicPickExpert._vector(observation, "target_zone_pos", 3)
        eef = HeuristicPickExpert._vector(observation, "robot0_eef_pos", 3)
        action = np.zeros(7, dtype=np.float32)
        action[-1] = self.config.open_command
