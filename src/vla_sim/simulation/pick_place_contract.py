...
def task_phase_prompt(task: str, group: str) -> str:
    """Return the canonical PickPlace phase prompt."""

    if task != PICK_PLACE_TASK:
        raise ValueError(f"Unsupported PickPlace task: {task}")
    prompts = {
        "approach": "move above the red cube",
        "grasp": "move down and grasp the red cube",
        "lift": "lift the red cube",
        "transport": "move the red cube above the blue storage bin",
        "place_release": "place the red cube in the blue storage bin and release",
        "failed": "hold safely",
    }
    if group not in prompts:
        raise ValueError(f"Unknown PickPlace phase group: {group}")
