...
from vla_sim.simulation.contracts import IMAGE_KEY, WRIST_IMAGE_KEY
...
from vla_sim.simulation.tasks import UR5ePickPlaceConfig, make_ur5e_pick_place
        ...
        json.dumps(
            {
                ...
                "checkpoint_config_sha256": sha256_file(_checkpoint_config_path(args.checkpoint)),
...
def _checkpoint_config_path(checkpoint: Path) -> Path:
    ...
    return config if config.is_file() else checkpoint / "adapter_config.json"
...
def _process_memory_counters() -> dict[str, int]:
    """Read process memory counters without adding a psutil dependency."""
    ...
    if sys.platform != "win32" or not hasattr(ctypes, "WinDLL"):
        ...
        try:
            ...
            return {"WorkingSetSize": rss, "PrivateUsage": private}
        except (OSError, ValueError, IndexError):
            import resource

            rusage = resource.getrusage(resource.RUSAGE_SELF)
            rss = int(rusage.ru_maxrss * 1024)
            return {"WorkingSetSize": rss, "PrivateUsage": rss}
