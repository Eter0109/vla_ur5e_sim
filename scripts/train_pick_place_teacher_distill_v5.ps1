param(
    ...
    [string]$Output = "outputs\pick_place_v2_native_bin\teacher_distill_transport_v5_1k\seed1000",
    [int]$Steps = 1000,
    [int]$BatchSize = 8,
    [int]$Seed = 1000
)
...
# Replay corrected teacher actions only during transport.
# v5: AuxiliarySampleWeight = 0.40 (teacher share within transport ~17%, total mass ~4.6%)
# Exact phase mass normalization fix in sampling.py is enabled.
& (Join-Path $PSScriptRoot "train_smolvla.ps1") `
    ...
    -AuxiliarySampleWeight "0.40" -AuxiliaryPhaseGroups "transport" `
    -Output $Output -Steps $Steps -BatchSize $BatchSize -NumWorkers 0 `
    -LearningRate 0.0000005 -WarmupSteps 100 -DecaySteps $Steps -DecayLR 0.0000002 `
    -ChunkSize 16 -ActionSteps 8 -SaveFreq 250 -Seed $Seed `
