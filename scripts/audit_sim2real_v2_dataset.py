...
from vla_sim.provenance import scene_seed_overlap
...
def _reference_manifests(task: str) -> list[Path]:
    patterns = {
        "push": "push_robust_*v1.json",
        "pick_place": "pick_place_robust_*v1.json",
        "color_pick": "color_pick_*v1.json",
    }
    collection_name = f"{task}_sim2real_v2_collection.json"
    return [
        path
        for path in (ROOT / "configs" / "benchmarks").glob(patterns[task])
        if path.name != collection_name and "collection" not in path.name
    ]
