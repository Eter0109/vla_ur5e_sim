"""Smoke-test the current UR5e Push simulator with the Push expert."""
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        for episode in range(args.episodes):
            ...
            front_shape = observation.get("observation.images.agentview", np.array([])).shape
            print(
                f"episode={episode} steps={step + 1} reward={total_reward:.3f} "
                f"success={success} front_shape={front_shape}"
            )
