...
$Steps = @("001000", "002000", "003000")
Push-Location $Root
try {
    foreach ($Step in $Steps) {
        $Checkpoint = Join-Path $RunRoot "checkpoints\$Step\pretrained_model"
        $Development = Join-Path $RunRoot "development"
        $Output = Join-Path $Development "screen6_${Step}_replan8_seed1000.json"
        $Log = Join-Path $Development "screen6_${Step}_replan8_seed1000.log"
        $ErrorLog = Join-Path $Development "screen6_${Step}_replan8_seed1000.err.log"
        if (Test-Path $Output) {
        ...
        try {
            & $Python scripts\run_pick_place_vla_only.py `
                ...
                --episodes 6 --replan-steps 8 --samples-per-plan 1 `
