"""Track 1: Run staged three-task LoRA training with ColorPick focus and Grip loss 3.0."""
...
# 实验 1: 强调困难任务与微小物体抓取咬合 (Color 40%, Push 30%, Pick 30%)
TASK_PROPORTIONS = {
    "push the block into the red target circle": 0.30,
    "place the red cube in the blue storage bin": 0.30,
    "pick up the blue cube": 0.40 / 3.0,
    "pick up the green cube": 0.40 / 3.0,
    "pick up the red cube": 0.40 / 3.0,
}
...
def _status(stage: str, **details) -> None:
    ...
    print(f"[Track 1] formal_training_stage={stage} details={details}", flush=True)


def _run(command: list[str], *, allow_failure: bool = False, extra_env: dict | None = None) -> int:
    ...
    if result.returncode and not allow_failure:
        raise RuntimeError(f"[Track 1] Command failed with exit code {result.returncode}: {command[1]}")
...
def _train_to(target_step: int) -> None:
    ...
    while True:
        ...
        if latest and latest[0] >= target_step:
            return
        if restart_count > 3:
            raise RuntimeError("[Track 1] Training failed after three automatic recovery attempts")
        ...
        if latest is not None:
            ...
            raise RuntimeError("[Track 1] Partial output exists without a complete checkpoint; refusing overwrite")
        _status(
        ...
        return_code = _run(
            command,
            allow_failure=True,
            extra_env={"VLA_GRIPPER_LOSS_WEIGHT": "3.0", "VLA_XYZ_LOSS_WEIGHT": "2.0"}
        )
...
def _evaluate_two_task_screen(step: int) -> bool:
    ...
    output = ROOT / f"outputs/sim2real_v2_exp1/eval_{step:06d}/screen"
...
def _evaluate_full(step: int) -> bool:
    checkpoint = _checkpoint(step) / "pretrained_model"
    output = ROOT / f"outputs/sim2real_v2_exp1/eval_{step:06d}/nominal_full"
    output.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    STATUS.parent.mkdir(parents=True, exist_ok=True)
    lock_path = STATUS.parent / ".sim2real_v2_exp1_supervisor.lock"
