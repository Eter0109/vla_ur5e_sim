...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--auxiliary-dataset", default="")
    parser.add_argument("--auxiliary-repo-id", default="")
    parser.add_argument("--auxiliary-sample-weight", default="1.0")
    parser.add_argument("--auxiliary-task-prompts", default="")
    parser.add_argument(
        "--lora-rank",
        type=int,
        default=0,
...
def main() -> int:
    ...
    if bool(args.auxiliary_dataset) != bool(args.auxiliary_repo_id):
        raise ValueError("auxiliary dataset and repo id must be supplied together")
    ...
    environment.update(
        {
            ...
            "VLA_PHASE_BALANCED": os.environ.get("VLA_PHASE_BALANCED", "0"),
            "VLA_TRANSITION_OVERSAMPLE_FACTOR": os.environ.get("VLA_TRANSITION_OVERSAMPLE_FACTOR", "1.0"),
            "VLA_TRANSITION_OVERSAMPLE_WINDOW": os.environ.get("VLA_TRANSITION_OVERSAMPLE_WINDOW", "0"),
            "VLA_AUXILIARY_DATASET": args.auxiliary_dataset,
            "VLA_AUXILIARY_REPO_ID": args.auxiliary_repo_id,
            "VLA_AUXILIARY_SAMPLE_WEIGHT": args.auxiliary_sample_weight,
            "VLA_AUXILIARY_TASK_PROMPTS": args.auxiliary_task_prompts,
            "VLA_CONTINUE_PEFT_ADAPTER": "1" if args.continue_peft_adapter else "0",
            "VLA_TASK_SAMPLING_PROPORTIONS": json.dumps(task_sampling_proportions),
        }
    )
    ...
    command = [
        sys.executable,
        str(ROOT / "scripts" / "train_entrypoint.py"),
    ...
    manifest = {
        ...
        "auxiliary_repo_id": args.auxiliary_repo_id,
        "auxiliary_sample_weight": args.auxiliary_sample_weight,
