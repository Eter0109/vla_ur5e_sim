param(
    ...
    [string]$Output = "outputs\pick_place_v2_native_bin\teacher_distill_transport_v5_1_600\seed1000",
    [int]$Steps = 600,
    [int]$BatchSize = 8,
    [int]$Seed = 1000
)
...
# v5.1: AuxiliarySampleWeight = 0.35, LR = 8e-7, Steps = 600
& (Join-Path $PSScriptRoot "train_smolvla.ps1") `
    ...
    -AuxiliarySampleWeight "0.35" -AuxiliaryPhaseGroups "transport" `
    -Output $Output -Steps $Steps -BatchSize $BatchSize -NumWorkers 0 `
    -LearningRate 0.0000008 -WarmupSteps 100 -DecaySteps $Steps -DecayLR 0.0000003 `
